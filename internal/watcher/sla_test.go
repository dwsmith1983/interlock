...
func TestParseSLADeadline_Duration(t *testing.T) {
	...
	require.NoError(t, err)
	// Duration means 2h after midnight today
...
func TestParseSLADeadline_InvalidFormat(t *testing.T) {
	...
	_, err := ParseSLADeadline("not-a-time", "", now)
	assert.Error(t, err)
}
...
func TestParseSLADeadline_InvalidTime(t *testing.T) {
	...
	_, err := ParseSLADeadline("25:00", "", now)
	assert.Error(t, err)
}
...
func TestIsBreached(t *testing.T) {
	...
	assert.False(t, IsBreached(deadline, deadline)) // exactly at deadline is not breached
}
