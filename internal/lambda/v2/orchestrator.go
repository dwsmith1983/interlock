...
func handleEvaluate(ctx context.Context, d *Deps, input OrchestratorInput) (OrchestratorOutput, error) {
	...
	if result.Passed {
		_ = publishEvent(ctx, d, string(v2types.EventValidationPassed), input.PipelineID, input.ScheduleID, input.Date, "all validation rules passed")
	}
...
// handleTrigger builds a v1 TriggerConfig from the v2 JobConfig, executes it,
...
func handleTrigger(ctx context.Context, d *Deps, input OrchestratorInput) (OrchestratorOutput, error) {
	...
	_ = publishEvent(ctx, d, string(v2types.EventJobTriggered), input.PipelineID, input.ScheduleID, input.Date, fmt.Sprintf("triggered %s job", cfg.Job.Type))
...
// buildTriggerConfig converts a v2 JobConfig into a v1 TriggerConfig by
// JSON-marshalling the config map and unmarshalling it into the typed sub-struct.
func buildTriggerConfig(job v2types.JobConfig) (types.TriggerConfig, error) {
