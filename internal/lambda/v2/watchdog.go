...
func detectStaleTriggers(ctx context.Context, d *Deps) error {
	...
	for _, tr := range triggers {
		...
		_ = publishEvent(ctx, d, string(v2.EventSFNTimeout), pipelineID, schedule, date,
...
func isStaleTrigger(tr v2.ControlRecord, now time.Time) bool {
	if tr.TTL > 0 {
...
func parseTriggerRecord(tr v2.ControlRecord) (pipelineID, schedule, date string, err error) {
...
func detectMissedSchedules(ctx context.Context, d *Deps) error {
	...
	for id, cfg := range configs {
		...
		_ = publishEvent(ctx, d, string(v2.EventScheduleMissed), id, scheduleID, today,
